...
TEST_CASE("Addition") {
    const char* test = "3+5";
    REQUIRE(shuntingYard(test) == 8);
}

TEST_CASE("Subtraction") {
    const char* test = "3-5";
    REQUIRE(shuntingYard(test) == (-2));
}

TEST_CASE("Multiplication") {
    const char* test = "3*5";
    REQUIRE(shuntingYard(test) == 15);
}

TEST_CASE("Division") {
    const char* test1 = "3/5";
    const char* test2 = "5/3";
    REQUIRE(shuntingYard(test1) == 0);
    REQUIRE(shuntingYard(test2) == 1);
}
...
TEST_CASE("Greater than") {
    const char* test1 = "5 > 3";
    REQUIRE (shuntingYard(test1) == 1);

    const char* test2 = "3 > 5";
    REQUIRE (shuntingYard(test2) == 0);
}
...
TEST_CASE("Equal") {
    ...
    REQUIRE_THROWS (shuntingYard(test3));
}
...
TEST_CASE("If") {
    ...
    REQUIRE (shuntingYard(test1) == 2);

    const char* test2 = "if 5==3 ? 2 : 3;";
    REQUIRE (shuntingYard(test2) == 3);
}


TEST_CASE("Bracket expressions") {
    const char* test1 = "(3+5)";
    REQUIRE(shuntingYard(test1) == 8);

    const char* test2 = "(3-5)";
    REQUIRE(shuntingYard(test2) == (-2));

    const char* test3 = "(3*5)";
    REQUIRE(shuntingYard(test3) == 15);

    const char* test4 = "(3/5)";
    const char* test5 = "(5/3)";
    REQUIRE(shuntingYard(test4) == 0);
    REQUIRE(shuntingYard(test5) == 1);
}

TEST_CASE("Empty brckets") {
    const char* test = "()";
    REQUIRE_THROWS(shuntingYard(test));
}

TEST_CASE("Mismatched brckets") {
    const char* test1 = "3+5)";
    REQUIRE_THROWS(shuntingYard(test1));

    const char* test2 = "(3+5";
    REQUIRE_THROWS(shuntingYard(test2));
}

TEST_CASE("Complex expressions - value") {
    const char* test1 = "5/3*(5/2)-5*(4*3/(2))";
    REQUIRE (shuntingYard(test1) == (-20));
}
